...
echo "#####################################################################################################################"
echo "                                                                                                                "
echo "            _____        ______  _______  _________________     ______        _____          ____       _____   "
...
echo "    |     |    |    |     |  \|_|/ /    / |    |   |       |     /___/| |    |   |    |    .--.    |    |\    \ "
echo "    |\     \  /    /|     |       |    |  |   /   //       |     \____|\|    |   |_,  |    |  |    |    | |    |"
echo "    | \_____\/____/ |\____\       |____|  /  /___//        |____ '     /|\ ___\___/  /|____|  |____|____| |____|"
echo "     \ |    ||    | / |    |      |    | /   |   |         |    /_____/ | |   /____ / |    |  |    |    | |    |"
echo "      \|____||____|/ \|____|      |____|/    |___|         |____|     | /\|___|    | /|____|  |____|____| |____|"
echo "         \(    )/       \(          )/        \(             \( |_____|/   \( |____|/   \(      )/   \(     )/  "
echo "          '    '         '          '          '              '    )/       '   )/       '      '     '     '   "
echo "                                                                   '            '   Fiture All Repository !!!   "
echo "                                                                                                                "
echo "#####################################################################################################################"
sleep 2

PS3='Please enter your choice: '
options=("Install Web Dinamis" "Install Web Static" "Install Proftpd" "Auto Mount EBS" "Quit")
select opt in "${options[@]}"; do
    case $opt in
    "Install Web Dinamis")
        echo "Select Your Apps"
        dinamis=("Datasiswa Apps" "Batiku" "Quit")
        select pil in "${dinamis[@]}"; do
            ...
            "Datasiswa Apps")
                ...
                clear
                echo "kita akan configurasi file koneksi dari apps ini"
                nano /var/www/web-dinamis-produktif/asset/koneksi.php
...
EOF
                ;;
            "Batiku")
                pilbatiku=("Untuk Admin" "Untuk Client")
                select bat in "${pilbatiku[@]}"; do
                    case $bat in
                        "Untuk Admin") 
                        ...
                        cp 000-default.conf ../sites-enabled/
                        systemctl restart apache2
                        sed -i 's/localhost/database-1.csfoslsqwvcb.us-east-1.rds.amazonaws.com/' /var/www/batiku/config/db.php
